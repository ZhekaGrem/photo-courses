...
const BottomTabs = () => {
  ...
  return (
    <>
      {/* prettier-ignore */}
      <div className="sticky bottom-4 left-1/2  z-10 w-full max-w-xs px-3 lg:-translate-x-1/2  ">
      <nav className="rounded-full bg-background_btn_burger animate-tilt-shaking ">
        <div className="flex max-w-screen-xl flex-wrap items-center justify-between p-2">
          <ul className="flex w-full justify-between">
            {tabs.map((tab, id) => (
              <li key={id} className="font-bold text-white hover:shadow-xl ">
                <button onClick={() => handleVariantChange(tab.id)}>
                  <Link
                    href={tab.href}
                    className={`block rounded-2xl p-1 text-center text-xs font-medium shadow-lg transition duration-300 ease-in-out md:p-2 ${
                      variantId  === tab.id
