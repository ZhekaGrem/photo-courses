    ...
    <form
      ...
      <div className="relative h-11 w-full min-w-[200px]">
        <input
          ...
          className={`peer h-full w-full border-solid bg-transparent pb-1.5 pt-4 font-sans text-sm font-normal outline-none transition-all focus:border-b-2 focus:outline-none disabled:border-0 ${
            errors.name || (isTouched.name && !errors.name) ? 'border-red-500' : ''
        ...
        <label
          className={`after:content[' '] pointer-events-none absolute -top-1 left-0 flex h-full w-full select-none text-sm font-normal leading-tight ${
            errors.name || (isTouched.name && !errors.name) ? '' : 'text-white'
          }`}>
          Ваше ім'я:{' '}
        </label>
      ...
      <div className="relative h-11 w-full min-w-[200px]">
        <input
          ...
          type="tel"
          className={`peer h-full w-full border-solid bg-transparent pb-1.5 pt-4 font-sans text-sm font-normal outline-none transition-all focus:border-b-2 focus:outline-none disabled:border-0 ${
            errors.tel || (isTouched.tel && !errors.tel) ? 'border-red-500' : ' '
          } ${isSubmitted && !errors.tel ? 'border-green-500' : ''}`}
        />
        <label
          className={`after:content[' '] pointer-events-none absolute -top-1 left-0 flex h-full w-full select-none text-sm font-normal leading-tight ${
            errors.tel || (isTouched.tel && !errors.tel) ? '' : 'text-white'
          }`}>
          Ваш номер
        </label>
      ...
      <div className="relative h-11 w-full min-w-[200px]">
        ...
        <label
          className={`after:content[' '] pointer-events-none absolute -top-1 left-0 flex h-full w-full select-none text-sm font-normal leading-tight ${
            errors.email || (isTouched.email && !errors.email) ? '' : 'text-white'
          }`}>
          Ваш емейл{' '}
        </label>
        {(errors.email || (isTouched.email && !errors.email)) && (
          <p className="absolute text-left text-sm">
            {(errors.email?.message as string) || 'email не є обовязковим'}
          </p>
        )}
      </div>
      ...
      <div className="flex justify-center gap-10">
        <button
          type="submit"
          className="rounded-3xl border-4 border-double border-transparent p-2 text-2xl font-bold text-white hover:border-white focus:border-white disabled:text-gray-700 disabled:hover:border-gray-600 md:text-xl">
          Зв`язатись
        </button>
      </div>
    </form>
